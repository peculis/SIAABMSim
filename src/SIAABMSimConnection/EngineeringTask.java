...
public class EngineeringTask extends Task{
	...
	public double actorUsefulKnowledgeRatio(){
		return thisTEOW.actorUsefulKnowledgeRatio((Actor) fromThisAgent);
	}
	...
	public boolean canActorDoIt(){
		return thisTEOW.canActorDoIt((Actor) fromThisAgent);
	}

	public void assign(Actor actor){
		fromThisAgent = actor;
		state = TaskState.ASSIGNED;
		thisTEOW.sendForExecution();
	...
	public void estimate(Actor actor){
		thisTEOW.estimate(actor);
	}
	
	public void estimate(){
		thisTEOW.estimate((Actor) fromThisAgent);
	}
	...
	public void draw(SimGraphics G){
		...
		if(state == TaskState.INPROGRESS){
			G.drawLink(Color.GREEN, (int) xScale * fromThisAgent.getX(), (int) xScale * toThatAgent.getX(), 
					(int) yScale * fromThisAgent.getY(), (int) yScale * toThatAgent.getY());
		}
	}
